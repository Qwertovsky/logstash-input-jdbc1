...
describe LogStash::Inputs::Jdbc1 do
  ...
  let(:settings) { {} }
  let(:plugin) { LogStash::Inputs::Jdbc1.new(mixin_settings.merge(settings)) }
  let(:queue) { Queue.new }
  let (:db) do
  ...
  context "update the previous default last_run_metadata_path" do
    ...
    context "when a file exists" do
      ...
      it "should be moved" do
        ...
        path = LogStash::SETTINGS.get_value("path.data")
        full_path = "#{path}/plugins/inputs/jdbc1/.logstash_jdbc_last_run"
